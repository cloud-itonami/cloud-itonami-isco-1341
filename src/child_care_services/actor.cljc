(ns child-care-services.actor
  "ChildCareServicesActor — the ISCO-08 1341 independent child-care-
  services-management actor as a `langgraph.graph/state-graph` (per
  ...
  ChildCareServicesGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [child-care-services.advisor :as advisor]
            [child-care-services.governor :as governor]
            [child-care-services.store :as store]))

(defn build-graph
  "Build a compiled ChildCareServicesActor graph. `store` implements
  `child-care-services.store/Store`. `advisor` implements
  `child-care-services.advisor/Advisor` (defaults to `mock-advisor`).
